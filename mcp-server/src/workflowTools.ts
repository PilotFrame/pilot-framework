...
function buildWorkflowExecutionGuide(
  workflow: WorkflowDefinition,
  personas: Map<string, PersonaSpec>
): string {
  ...
  for (const step of sortedSteps) {
    ...
    if (persona) {
      const spec = persona.specification as Record<string, unknown>;
      const mission = spec.mission as string;
      const inputs = spec.inputs as string[] | undefined;
      const handoffExpectations = spec.handoff_expectations as string[] | undefined;
      ...
      if (spec.workflow && Array.isArray(spec.workflow)) {
        ...
        const workflowSteps = spec.workflow as string[];
...
export async function registerWorkflowTools(server: McpServer, personas: PersonaSpec[]) {
  const definitions = await loadWorkflowsFromControlPlane();
  
  // Create a map for quick persona lookup
  const personaMap = new Map<string, PersonaSpec>();
    ...
    server.registerTool(
      ...
      async ({ input } = {}) => {
        ...
        const stepDetails = sortedSteps.map((step) => {
          ...
          const spec = persona?.specification as Record<string, unknown> | undefined;
          return {
            step_id: step.id,
            order: step.order,
            ...
            handoff_expectations: spec?.handoff_expectations as string[] | undefined,
        ...
        return {
          ...
          structuredContent: {
            ...
            description: workflow.metadata?.description as string | undefined,
            steps: stepDetails,
