...
export function AssistantPage({ config, connectionStatus }: AssistantPageProps) {
  ...
  }, [canCallApi, config, loadConversations]);
  ...
  const handleSaveSpec = useCallback(async () => {
    ...
    try {
      ...
      const endpoint =
        suggestedSpec.type === 'persona' ? '/api/personas' : '/api/workflows';
      const response = await fetch(new URL(endpoint, config.baseUrl).toString(), {
        ...
        body: JSON.stringify(suggestedSpec.spec)
      });
  ...
  const handleCopyToEditor = useCallback(() => {
    ...
    sessionStorage.setItem('assistantGeneratedSpec', JSON.stringify(suggestedSpec.spec));
    sessionStorage.setItem('assistantSpecType', suggestedSpec.type);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Suggested Spec Preview */}
      {suggestedSpec && (
        <div className="rounded-lg border border-green-800 bg-green-900/20 p-6">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-green-400">
              {suggestedSpec.type === 'persona' ? 'Persona' : 'Workflow'} Ready to Save
            </h3>
